...
This document describes the standard release process for dolphinscheduler-operator.
...
4. **Helm chart publish** — pushes to
   `quay.io/kubedoopcharts/dolphinscheduler-operator:<version>` (OCI registry)
...
"New branch", name it `release-0.4` and base it on `main`.

**Via GitHub API / gh CLI:**

```bash
gh api repos/zncdatadev/dolphinscheduler-operator/git/refs \
  -f ref=refs/heads/release-0.4 \
  -f sha=$(gh api repos/zncdatadev/dolphinscheduler-operator/git/ref/heads/main --jq .object.sha)
```
...
git tag x.y.z-dev upstream/release-0.4
...
- Docker image is available at
  `quay.io/zncdatadev/dolphinscheduler-operator:x.y.z-dev`
- Helm chart is available at
  `quay.io/kubedoopcharts/dolphinscheduler-operator:x.y.z-dev`
...
```bash
git pull --rebase upstream release-0.4
git tag x.y.z upstream/release-0.4
...
- **Chainsaw Test** — Runs Chainsaw E2E tests across multiple Kubernetes and DolphinScheduler versions
- **Release Image** — Builds and pushes multi-arch Docker image using the root
  Dockerfile to `quay.io/zncdatadev/dolphinscheduler-operator:<version>`, and signs the
  image with Cosign

## Versioning Convention

dolphinscheduler-operator follows [Semantic Versioning](https://semver.org):
...
Here is an example of releasing version `0.4.4` on the `release-0.4` branch:
...
git pull --rebase upstream release-0.4
git tag 0.4.4-dev upstream/release-0.4
git push upstream 0.4.4-dev
# Wait for workflow to pass

# Step 4: Tag and publish (stable version, can only be published once)
git pull --rebase upstream release-0.4
git tag 0.4.4 upstream/release-0.4
git push upstream 0.4.4
```
